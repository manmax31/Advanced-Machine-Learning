...
References: Notes from Prof. Sebastian Thrun's Artificial Intelligence for Robotics
...
def main():
    ...
    for measurement, odo in zip(measurements, odometry):
        ...
        #=======================================================================
        #print( sorted(range(len(weights)), key=lambda i: weights[i], reverse=True)[:2] )
        #indices =  sorted(range(len(weights)), key=lambda i: weights[i], reverse=True)[:50]

        #mostLikelyParticle = getMostLikelyParticle(particles, weights, 1)
        mostLikelyParticles.append( getMostLikelyParticle(particles, weights, 1) )
    ...
    canvasImg = visualize(wnd, mostLikelyParticles, map)
    canvasImg.save("Final image.jpg")
